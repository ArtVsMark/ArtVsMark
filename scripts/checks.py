"""Свёртка записей check-runs по имени — в одном месте на весь репозиторий.

Зачем отдельный модуль ради нескольких строк. Правило каталога 009 говорит:
считать уникальные имена, а не записи. Оно было известно, помечено в
`.rules/bindings.json` как действующее — и всё равно нарушено в новом коде:
сборка метрик сворачивала записи правильно, а конвейер слияния, написанный
позже, сравнивал исход каждой записи и считал незелёным PR, у которого рядом со
свежей успешной проверкой висела отменённая.

Знание правила не помогло, потому что механизма не было: свёртка жила
выражением внутри функции, и второй потребитель написал своё. Это 090 —
общая утилита выносится **выше обеих** подсистем.

GitHub отдаёт по одному имени столько записей, сколько раз проверка
запускалась: отмена предыдущего прогона при новом пуше, повтор после
обновления ветки, ручной перезапуск. История событий, а не текущее состояние.

**Потребитель остался один.** Второй — собственный конвейер слияния — исчез
вместе со скриптом: слияние отдано штатному автомержу площадки, и «зелено ли
здесь» решает теперь она, по обязательным проверкам защиты ветки. Вместе с ним
ушли и функции, отвечавшие на этот вопрос: `latest_by_name` и `red_names`. Их
знание — «свежая запись, а бегущая не считается зелёной» — записано в
`HISTORY.md` и в правиле 009, а код, который никто не зовёт и ничто не
проверяет, хуже отсутствующего.

Модуль при этом остался, хотя потребитель один: правило про уникальные имена
стоило витрине неверной цифры на странице, и место, где оно названо по имени,
дороже трёх строк экономии.

Запуск::  python scripts/checks.py --selftest
Исходы: 0 — чисто; 1 — самопроверка провалена; 2 — запустить не удалось
(модуль не разобрался — это видно трассировкой импорта, а не кодом).
"""

from __future__ import annotations

import os
import re
import subprocess
import sys


#: Сводка каталога о своих потребителях: чем у каждого держится каждое правило.
#:
#: ВЕТКА ЗДЕСЬ ЧАСТЬ АДРЕСА, А НЕ ПОДРОБНОСТЬ. Сводка лежит на `badges`, а не в
#: общей ветке — так сказано в контракте каталога (export/README.md). Ошибка
#: стоит дорого и молча: `main` отвечает 404, и читающий решает, что предмета
#: нет вовсе.
#:
#: ПОЧЕМУ ОДНО МЕСТО (правило 090), И ЧЕМ ЭТО ОПЛАЧЕНО. Копий было ДВЕ, и они
#: разъехались на чужой правке. 2 сентября подсказка соседей завелась с адресом
#: в `main` — и он был ВЕРНЫЙ: первый прогон ответил по всем шестнадцати
#: пробелам. 4 сентября каталог перенёс сводку на `badges` («сводка ушла на
#: ветку badges — в общей ветке её нет»). 8 сентября сборка метрик завела свою
#: копию — уже с `badges` — и назвала прежнюю ошибку в комментарии, но вторую
#: копию не поправила: починка не назвала соседей (правило 195).
#:
#: ЧЕТЫРЕ СУТОК НЕЗАМЕЧЕННОГО 404 ОБЪЯСНЯЮТСЯ ДВУМЯ ПРИЧИНАМИ, и обе названы:
#: долг по правилам стал нулевым, и подсказка обрывалась раньше похода в сеть,
#: а шаг в pr-check стоит с `continue-on-error` — отказ там не красит ничего
#: (правило 146: зелёное здесь означало только, что спрашивать было не о чем).
CATALOGUE_WHERE = ("https://raw.githubusercontent.com/ArtVsMark/"
                   "Engineering-Incidents-Playbook/badges/export/where.json")


def git_paths(*args: str, cwd=None) -> list[str]:
    """Пути из git, прочитанные по NUL, а не по строкам.

    БЕЗ ``-z`` git ЭКРАНИРУЕТ имена с не-ASCII символами и пробелами: заключает
    в кавычки и выводит байты восьмеричными последовательностями. Построенный из
    такой строки путь не разрешается, и файл молча выпадает из обработки —
    проверка остаётся зелёной, ничего не увидев.

    Воспроизведено на подделанном дереве: три файла в ``assets/`` —
    ``normal-dark.svg``, ``утечка-dark.svg`` и ``с пробелом.svg``. Разбор по
    строкам с фильтром ``endswith(".svg")`` увидел ОДИН: у остальных строка
    кончается кавычкой, а не расширением. ``.split()`` в разборе диапазона был
    хуже вдвое — он рвал по пробелам и путь с пробелом превращал в два.

    ПОМОЩНИК ОБЩИЙ, А НЕ ТРИ КОПИИ (правило 090): пути из git читают гейт ролей,
    гейт журнала и сверка производного, и разъехались бы они молча — первый же
    исправленный оставил бы два слепых.

    Возвращает список путей. Сколько их — печатает вызывающий: без числа
    слепота проверки неотличима от чистого результата (правило 165).
    """
    command = ["git"]
    if cwd is not None:
        command += ["-C", str(cwd)]
    out = subprocess.run([*command, *args, "-z"], capture_output=True,
                         text=True, encoding="utf-8", check=True).stdout
    return [path for path in out.split("\0") if path]


#: Кто не принимает решений и потому не отвечает за них. Список закрытый и
#: короткий: освобождение от гейта — не «бот, значит можно», а признание, что
#: требовать нечего. Новый бот придётся вписать сюда вслух (правило 051).
#:
#: Проверяется по АВТОРУ коммита, а не по имени ветки: имя ветки бот выбирает
#: сам и может сменить формат, а учётная запись площадки устойчива и видна
#: и гейту журнала (у него есть коммиты), и гейту имени (у него есть только
#: ветка) — второму через префикс ниже.
MACHINE_AUTHORS = ("dependabot[bot]", "github-actions[bot]")

#: Приставки веток, которые заводит не человек.
MACHINE_BRANCHES = ("dependabot/", "chore/metrics")

#: ЧТО СЧИТАЕТСЯ ПРАВКОЙ ПОВЕДЕНИЯ. Список короткий намеренно: `.rules/` сюда не
#: входит — ответ каталогу меняется и от чужой правки, а `README.md` собирается
#: сборкой, и требовать чего-либо от машинной правки значило бы требовать этого
#: от расписания.
#:
#: ЖИВЁТ ЗДЕСЬ, А НЕ В ГЕЙТЕ, потому что спрашивают его ДВОЕ: журнал требует
#: записи о решении, соседи — перечня того, что решал изменённый признак. Две
#: копии одного списка разошлись бы на первой же правке, и разошлись бы молча
#: (правило 090).
BEHAVIOUR = (re.compile(r"^scripts/.*\.py$"),
             re.compile(r"^\.github/workflows/.*\.ya?ml$"))


def touched(paths: list[str]) -> list[str]:
    """Файлы поведения среди изменённых."""
    return [p for p in paths if any(rx.match(p) for rx in BEHAVIOUR)]


def machine_made(author: str) -> bool:
    """Коммит сделан не человеком. Пусто/чужое имя — считаем человеком.

    Умолчание в сторону человека намеренно: ошибиться и потребовать журнал у
    живого автора дешевле, чем молча освободить от него всех.

    ПРИНИМАЮТСЯ ОБЕ ФОРМЫ ПОДПИСИ — голое имя (``%an``) и имя с адресом
    (``%an <%ae>``), потому что зовут отсюда из трёх мест и берут подпись
    по-разному: гейт журнала — голым именем, гейт тела — логином площадки,
    гейт трейлеров — строкой с адресом. Разбор одной формы из двух молчал
    ровно так же, как отсутствие выреза: 17 сентября #209 встало с отказом на
    коммите, автор которого `dependabot[bot] <49699333+dependabot[bot]@…>`, —
    имя совпадало, а строка целиком нет.

    АДРЕС ПРИ СРАВНЕНИИ НЕ УЧАСТВУЕТ: учётная запись площадки узнаётся по
    имени, и адрес у одного и того же бота уже бывал разным (у `github-actions`
    площадка пишет то с адресом, то без — замер в .github/authors.txt).
    """
    name = author.strip()
    if name.endswith(">") and " <" in name:
        name = name[:name.rindex(" <")].strip()
    return name in MACHINE_AUTHORS


#: Знак обрыва. Один на весь репозиторий: восемь мест обрезали вывод каждое
#: по-своему, а половина — вовсе без знака (правило 016).
ELLIPSIS = "…"


def clip(text: str, width: int) -> str:
    """Строка не длиннее ``width``, и если обрезана — это ВИДНО.

    ЗАЧЕМ ОБЩИЙ ПОМОЩНИК РАДИ ТРЁХ СТРОК. Урезанный вывод, не сказавший, что он
    урезан, выглядит ПОЛНЫМ — правило 016, и предмет был живой: шесть мест в
    гейтах витрины резали тему коммита, причину вердикта и текст чужого отказа
    голым срезом. Читатель видел законченную фразу и не знал, что у неё есть
    продолжение; у `--queue` так обрывались причины ответов на 96-м знаке.

    Восемь копий разошлись бы молча — знак обрыва у каждой свой (правило 090).

    Ширина считается ПО ИТОГУ: результат не длиннее ``width`` вместе со знаком,
    иначе колонка, ради которой обрезали, разъезжается на один символ.
    """
    if width <= 0:
        raise ValueError(f"ширина {width} не положительна — резать не по чему")
    if len(text) <= width:
        return text
    return text[: max(0, width - len(ELLIPSIS))] + ELLIPSIS


def tail(items: list[str], shown: int, sep: str = ", ") -> str:
    """Первые ``shown`` через запятую, и НАСКОЛЬКО список урезан.

    Отличается от ``clip`` предметом: там обрывается строка, здесь — перечень, и
    читателю нужно не многоточие, а ЧИСЛО непоказанного. «a, b, c» и «a, b, c и
    ещё 47» — разные утверждения о состоянии, и первое из них ложное.
    """
    if shown <= 0:
        raise ValueError(f"показать {shown} — перечень без единого элемента не отвечает ни о чём")
    head = sep.join(items[:shown])
    left = len(items) - shown
    return f"{head} и ещё {left}" if left > 0 else head


#: Уровни площадки. Список закрытый: команда с чужим словом площадкой
#: игнорируется молча, и находка не доехала бы никуда.
LEVELS = ("error", "warning", "notice")


#: Строка хвостового блока: `Ключ: значение`. Ключ без пробелов — так его
#: разбирает и сам git, и разбор каталога.
TRAILER_LINE = re.compile(r"^([A-Za-z][A-Za-z0-9-]*):\s*(.+?)\s*$")


def trailers(body: str) -> dict[str, list[str]]:
    """Трейлеры сообщения — из ХВОСТОВОГО блока, а не из любой строки.

    Правило каталога 156, и оно оплачено дважды. У каталога гейт атрибуции
    отверг изменение витрины #95, найдя соавтора «github-actions[bot] в
    уплотнённый коммит. #83 сделал автором»: слово `Co-authored-by` стояло в
    ПРОЗЕ и после переноса строки оказалось первым в ней. Такого соавтора не
    существует — это середина фразы.

    Цена ошибки несимметрична: разбор по образцу «строка начинается с имени»
    отвергает изменение тем охотнее, чем добросовестнее описан предмет.

    Хвостовой блок — последний абзац, у которого **все** непустые строки имеют
    форму `Ключ: значение`. Абзац, где хоть одна строка прозаическая, блоком не
    считается целиком: иначе разбор снова начинает угадывать.

    Помощник живёт здесь, а не в двух гейтах: тот же приём нужен и проверке
    авторства коммитов, и решению о стоп-кране, и вторая копия разошлась бы с
    первой молча (правило 090).
    """
    paragraphs = [block for block in re.split(r"\n\s*\n", body or "") if block.strip()]
    if not paragraphs:
        return {}
    lines = [line for line in paragraphs[-1].split("\n") if line.strip()]
    parsed = [TRAILER_LINE.match(line) for line in lines]
    if not all(parsed):
        return {}
    found: dict[str, list[str]] = {}
    for match in parsed:
        found.setdefault(match.group(1).lower(), []).append(match.group(2))
    return found


def annotate(level: str, text: str) -> str:
    """Строка находки — командой площадки в прогоне, обычной строкой вне его.

    ЗАЧЕМ ЭТО ЗДЕСЬ, А НЕ В КАЖДОМ ГЕЙТЕ. Замер правила 151: окну витрины логи
    прогонов закрыты — ``GET /actions/runs/<id>/logs`` уходит на хранилище,
    куда прокси отвечает отказом. До окна доходят исход каждого шага и
    аннотации check-run, а аннотация появляется только у того, что напечатано
    командой площадки. Гейт, печатающий причину в обычный поток, на чужом
    красном оставляет ровно «Process completed with exit code 1».

    ПОЧЕМУ НЕ ВСЕГДА С ПРЕФИКСОМ. Вне прогона ``::error::`` — шум перед
    сообщением, которое человек и так читает. Признак берётся у площадки
    (``GITHUB_ACTIONS``), а не выводится из наличия токена: токен бывает и на
    машине разработчика.

    Помощник вынесен ВЫШЕ гейтов, а не скопирован в каждый (правило 090): у
    восьми копий разъехался бы формат, и молча — площадка не жалуется на
    команду, которой не знает, она её просто не показывает.
    """
    if level not in LEVELS:
        raise ValueError(f"уровень {level!r} вне набора {LEVELS}")
    prefix = f"::{level}::" if os.environ.get("GITHUB_ACTIONS") == "true" else ""
    return f"{prefix}{text}"


def selftest() -> int:
    """Обе ветки ``annotate`` и отказ на чужом уровне (правила 140, 145).

    Набор ЗОВЁТ механизм и смотрит на его ответ, а не повторяет условие
    «если GITHUB_ACTIONS, то префикс» (правило 150): при смене признака
    случаи продолжат спрашивать функцию.
    """
    broken = []

    # ── хвостовой блок, а не любая строка (правило 156) ────────────────────
    # Обе ошибки названы: принять прозу за трейлер значит отвергнуть верное
    # изменение тем охотнее, чем подробнее оно описано; не увидеть настоящий
    # трейлер — пропустить коммит без атрибуции в общую ветку.
    TAIL = "Co-Authored-By: Claude Opus 5.5 <noreply@anthropic.com>"
    trailer_cases = [
        ("хвост из одного трейлера", f"Разбор.\n\n{TAIL}", ["co-authored-by"]),
        ("хвост из двух", f"Тело.\n\n{TAIL}\nClaude-Session: https://x", ["claude-session", "co-authored-by"]),
        ("слово в прозе — не трейлер",
         "площадка дописывает\nCo-Authored-By: github-actions[bot] в уплотнённый коммит", []),
        ("проза внутри хвостового абзаца отменяет весь блок",
         f"Тело.\n\n{TAIL}\nи ещё пояснение", []),
        ("трейлер в середине сообщения не считается",
         f"Начало.\n\n{TAIL}\n\nПродолжение прозой.", []),
        ("тела нет вовсе", "", []),
        ("одни пустые строки", "\n\n   \n", []),
        ("регистр ключа не важен", "Тело.\n\nco-authored-by: A <a@b>", ["co-authored-by"]),
        ("двоеточия нет — не трейлер", "Тело.\n\nCo-Authored-By A <a@b>", []),
    ]
    for name, body, expected in trailer_cases:
        got = sorted(trailers(body))
        if got != sorted(expected):
            broken.append(f"трейлеры, {name}: ожидалось {sorted(expected)}, вышло {got}")
        print(f"  {str(got) if got else 'пусто':<34} — трейлеры: {name}")

    # Значение доезжает целиком: по нему сверяют имя со списком согласованных.
    value = trailers(f"Тело.\n\n{TAIL}").get("co-authored-by", [""])[0]
    if value != "Claude Opus 5.5 <noreply@anthropic.com>":
        broken.append(f"трейлеры: значение обрезано или искажено — {value!r}")

    # ── кто не принимает решений, тот за них не отвечает (правило 051) ────
    machine_cases = [
        ("dependabot", "dependabot[bot]", True),
        ("прогон площадки", "github-actions[bot]", True),
        ("живой автор", "ArtVsMark", False),
        ("пустое имя — считаем человеком", "", False),
        ("похожее имя не считается", "dependabot", False),
        ("пробелы по краям не мешают", "  dependabot[bot]  ", True),
        # Вторая форма подписи — с адресом. Обе стороны: имя бота с адресом
        # машинное, похожее имя с адресом — нет.
        ("имя с адресом", "dependabot[bot] <49699333+dependabot[bot]@users.noreply.github.com>", True),
        ("прогон площадки с адресом",
         "github-actions[bot] <41898282+github-actions[bot]@users.noreply.github.com>", True),
        ("живой автор с адресом", "ArtVsMark <arvs.markitanov@gmail.com>", False),
        ("похожее имя с адресом не считается", "dependabot <bot@example.com>", False),
        ("угловая скобка в имени без адреса", "dependabot[bot]", True),
    ]
    for name, author, expected in machine_cases:
        got = machine_made(author)
        if got is not expected:
            broken.append(f"машинный автор, {name}: ожидалось {expected}, вышло {got}")
        print(f"  {'машина ' if got else 'человек'} — автор: {name}")

    # ── урезанный вывод говорит, что он урезан (правило 016) ──────────────
    # Обе стороны: пропустить обрыв — показать обрубок как целое; поставить
    # знак там, где не резали, — соврать в другую сторону.
    clip_cases = [
        ("короче ширины — не трогаем", "abc", 10, "abc"),
        ("ровно по ширине — не трогаем", "abcde", 5, "abcde"),
        ("длиннее — знак обрыва виден", "abcdefgh", 5, "abcd…"),
        ("итог не длиннее ширины ВМЕСТЕ со знаком", "abcdefgh", 5, None),
        ("пустая строка", "", 5, ""),
    ]
    for name, text, width, expected in clip_cases:
        got = clip(text, width)
        if expected is not None and got != expected:
            broken.append(f"обрыв строки, {name}: ожидалось {expected!r}, вышло {got!r}")
        if len(got) > width:
            broken.append(f"обрыв строки, {name}: итог длиннее ширины — {got!r}")
        print(f"  {got!r:<12} — обрыв строки: {name}")

    tail_cases = [
        ("перечень целиком — числа не дописываем", ["a", "b"], 3, "a, b"),
        ("ровно по границе", ["a", "b"], 2, "a, b"),
        ("урезан — назван остаток", ["a", "b", "c", "d"], 2, "a, b и ещё 2"),
        ("пустой перечень", [], 2, ""),
    ]
    for name, items, shown, expected in tail_cases:
        got = tail(items, shown)
        if got != expected:
            broken.append(f"обрыв перечня, {name}: ожидалось {expected!r}, вышло {got!r}")
        print(f"  {got!r:<18} — обрыв перечня: {name}")

    # Ширина, при которой резать не по чему, — ошибка вызова, а не молчаливый
    # пустой ответ: пустая строка на месте темы коммита выглядит как «темы нет».
    for bad, fn in ((0, lambda: clip("abc", 0)), (-1, lambda: clip("abc", -1)),
                    (0, lambda: tail(["a"], 0))):
        try:
            fn()
        except ValueError:
            pass
        else:
            broken.append(f"обрыв: ширина {bad} принята молча")

    saved = os.environ.get("GITHUB_ACTIONS")
    try:
        os.environ.pop("GITHUB_ACTIONS", None)
        if annotate("error", "находка") != "находка":
            broken.append("вне прогона строка обросла префиксом — шум человеку")
        os.environ["GITHUB_ACTIONS"] = "true"
        for level in LEVELS:
            got = annotate(level, "находка")
            if got != f"::{level}::находка":
                broken.append(f"в прогоне уровень {level} даёт {got!r}")
        # Признак берётся у площадки дословно: «1» это не «true», и принять
        # его значило бы печатать команду там, где её никто не прочитает.
        os.environ["GITHUB_ACTIONS"] = "1"
        if annotate("error", "находка") != "находка":
            broken.append("чужое значение признака принято за прогон")
    finally:
        os.environ.pop("GITHUB_ACTIONS", None)
        if saved is not None:
            os.environ["GITHUB_ACTIONS"] = saved

    try:
        annotate("fatal", "находка")
    except ValueError:
        pass
    else:
        broken.append("чужой уровень принят — площадка проглотит команду молча")

    for line in broken:
        print(f"  {line}")
    print("  свёртка имён и разметка находок проверены" if not broken else "")
    return 1 if broken else 0


def main() -> int:
    if "--selftest" in sys.argv[1:]:
        return selftest()
    print(__doc__)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
