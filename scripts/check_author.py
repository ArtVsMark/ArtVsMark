...
#: Трейлеры, как их видит гейт каталога, — любой строкой, а не хвостовым
#: блоком. Совпадение образцов намеренное: проверка до слияния предсказывает
#: проверку после него, иначе одна из них лишняя.
BODY_TRAILERS = {
    "co-authored-by": re.compile(r"^Co-Authored-By:\s*\S", re.M | re.I),
    "claude-session": re.compile(r"^Claude-Session:\s*\S", re.M | re.I),
}


def _body_trailers(body: str) -> set[str]:
    """Какие трейлеры видны в теле — тем же способом, каким их увидит каталог."""
...
def body_attribution(body: str, author: str = "", branch: str = "") -> str:
    ...
    Суточную пересборку открывает `agent-pr` токеном ВЛАДЕЛЬЦА — автор изменения
    ...
    РАЗБОР ЗДЕСЬ ПОСТРОЧНЫЙ, А НЕ ХВОСТОВЫМ БЛОКОМ, И ЭТО НЕ ОСЛАБЛЕНИЕ. Гейт,
    стоящий ДО слияния, обязан проверять ровно то, что потребует гейт ПОСЛЕ
    него, — иначе он либо пропускает то, на чём тот покраснеет, либо отвергает
    верное. `check_attribution.py` каталога ищет `Co-Authored-By` и
    `Claude-Session` любой строкой (`re.M`), и здесь берётся тот же критерий.

    Предмет заставил: в этом окружении тело изменения ВСЕГДА оканчивается
    служебным футером «_Generated by Claude Code_», который дописывает
    площадка. При строгом разборе (правило 156 — хвостовой блок это последний
    абзац) трейлеры перестают быть последним абзацем, и гейт краснеет на теле,
    которое гейт каталога примет. Строгость к прозе живёт там, где предмет —
...
def selftest() -> int:
    ...
    body_cases = [
        ...
        # примет — значит и этот обязан принять, иначе один из двух лишний.
        ("футер площадки после хвоста",
