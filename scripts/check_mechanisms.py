...
#: Прогоны, чьи записи держат слияние. Список локальный НАРОЧНО: настоящий
#: живёт в защите ветки, вне репозитория, и его сверяет сборка
#: (build_metrics.py::unmet_contexts). Здесь нужен не полный список, а тот
#: файл, про который известно, что его запись обязательна.
REQUIRED_FLOWS = ("pr-check.yml",)

#: Условие УРОВНЯ РАБОТЫ — четыре пробела внутри `jobs:`. Условие шага стоит
#: глубже и записи не создаёт: пропущенный шаг остаётся внутри своей работы.
JOB_CONDITION = re.compile(r"^    if:", re.MULTILINE)


def required_job_conditions(flows: dict[str, str]) -> list[str]:
    """Условие у работы, чья запись обязательна (правило 187).

    ЧЕМ ЭТО ПЛАТИТ. Условие ``if`` не отменяет ПОЯВЛЕНИЯ работы в списке
    проверок: площадка создаёт работу, вычисляет условие и оставляет запись
    ``skipped``. Защита ветки сверяет ИМЯ записи, а не исход, и нейтральное
    засчитывает наравне с успешным.

    ЦЕНА ЗАПЛАЧЕНА 7 СЕНТЯБРЯ: изменение #140 слилось при КРАСНОЙ обязательной
    проверке — на голове лежали три записи с именем ``PR check``: ``failure``,
    ``skipped`` и ``cancelled``. Обе половины дефекта были осознанными
    починками прошлых инцидентов: условие завели, чтобы не будить прогон на
    метку конвейера, отдельную группу отмены — чтобы события меток не отменяли
    настоящую проверку. Вместе они отключили обязательную проверку целиком.

    Отбор идёт по ОТСТУПУ, а не по слову: условие шага стоит глубже и записи
    не создаёт — отвергать его значило бы запретить законное (051).
    """
    return [f"{name}: у работы с обязательной записью стоит условие. Пропущенная "
            f"работа оставляет запись «skipped», а защита ветки считает её "
            f"выполненным требованием: слияние отпускается при красной проверке "
            f"(187). Фильтруйте события, а не работу"
            for name in REQUIRED_FLOWS
            if JOB_CONDITION.search(_section(flows.get(name, ""), "jobs"))]
...
def claimed_rules(sources: dict[str, str], rules: dict[str, dict]) -> list[str]:
    """Механизм, назвавший правило своим, против ответа «этого у нас нет» (184).

    ЧТО ЗДЕСЬ ЗА УТВЕРЖДЕНИЕ. Скрипт, объясняющий своё поведение номером
    правила, объявляет: это правило у нас действует и держится вот этим кодом.
    Ответ ``not-applicable`` в ``.rules/bindings.json`` объявляет обратное.
    Оба верными быть не могут, и разрешается спор в пользу ФАКТА: механизм
    существует и работает, а ответ — проза, которую ничто не двигает.

    ЗАМЕР ПРИ ЗАВЕДЕНИИ, 8 сентября: 152 объявления на 60 правил, три
    расхождения — и все три разной природы, что и делает гейт нужным.

    * **078** («отмена — отдельный исход») — ответ смотрел на букву
      «терминальных статусов витрина не публикует», а обобщением пользовался
      ``hold.py``: отменённое не провал, пропущенное тоже, и различие между
      ними держит слияние;
    * **114** («миграция от текущей версии») — ответ отвечал про ступени
      схемы, а обобщение работало в ``check_author.py``: планка поднимается
      для нового, прошлое не переписывается;
    * **083** — расхождение оказалось не в вердикте, а в ДЕВЯТИ объявлениях:
      девять мест ссылались на «сгенерированное проверяют свойствами», имея
      в виду «третий исход называет предмет отказа», то есть 158. Вердикт по
      083 при перечитывании тоже сменился, но по другой причине — витрина
      принимает свои картинки свойствами, а не эталоном.

    Третий случай и есть ответ на вопрос «а вдруг гейт всегда прав против
    вердикта»: он не судит, кто ошибся, он находит противоречие. Чинить его
    можно с обеих сторон — и однажды из трёх чинилась именно сторона кода.

    ПОДДЕЛКИ ОТРИЦАТЕЛЬНОГО НАБОРА ПРОПУСКАЮТСЯ: номера в нём — данные,
    положенные туда, чтобы гейт их отверг.
    """
    found = []
    for name, source in sorted(sources.items()):
        skip = _selftest_lines(source)
        ...
        for rule_id, line_no in sorted(seen.items()):
            ...
            status = rules.get(rule_id, {}).get("status")
            if status in DENIED:
                found.append(
                    f"{name}: строка {line_no} объявляет правило {rule_id} своим, "
                    f"а ответ витрины — «{status}». Механизм сильнее ответа: либо "
                    f"вердикт смотрел на букву и не заметил обобщения, либо номер "
                    f"в объявлении чужой (184)")
    return found
...
def audit_pipeline_labels(sources: dict[str, str], flows: dict[str, str]) -> list[str]:
    ...
    ОТСУТСТВИЕ КОПИИ ПЕРЕСТАЛО БЫТЬ НАХОДКОЙ 7 СЕНТЯБРЯ, и это не смягчение.
    Фильтр убран целиком, и убран за дело: пропуск по `if` создавал на голове
    запись `skipped` с именем обязательной проверки, а нейтральная запись
    удовлетворяет защиту ветки наравне с зелёной. Изменение #140 слилось при
    КРАСНОЙ проверке — красное завершилось в 09:30:32, слияние прошло в
    09:52:14.

    Предмета у сверки не стало: копии нет, потому что нет фильтра. Гейт
    сверяет копию, КОГДА ОНА ЕСТЬ, и молчит, когда её нет, — иначе он требовал
    бы вернуть механизм, из-за которого сливалось непроверенное (051).
    """
    found: list[str] = []
    ...
    if not PIPELINE_IN_FLOW.search(flow):
        # Фильтра нет — сверять нечего. Список в коде при этом остаётся живым:
        # его читает сам scripts/check_labels.py, отделяя конвейерные метки от
        # содержания, и на это стоит собственный набор.
        return []
...
def selftest() -> int:
    ...
    # Правило без вердикта — долг разбора, и его считает другой механизм (177);
    # красить им этот гейт значило бы отвечать за чужой предмет.
    ANSWERS = {"078": {"status": "not-applicable"}, "011": {"status": "active"},
               "031": {"status": "rejected"}}
    claims = [
        ("механизм объявил правило, ответ — «предмета нет»",
         {"a.py": "# отменённое не провал (правило 078)\n"}, True),
        ("механизм объявил отвергнутое правило",
         {"a.py": "# волна исполнителей (правило 031)\n"}, True),
        ("механизм объявил действующее правило",
         {"a.py": "# имя работы сверяется (правило 011)\n"}, False),
        ("ответа на правило нет вовсе — не предмет этого гейта",
         {"a.py": "# сказано по правилу 999\n"}, False),
        ("номер вне объявления — не объявление",
    ...
    WITH_STEP_IF = ("jobs:\n  check:\n    steps:\n      - run: echo\n"
                    "        if: github.event_name == 'pull_request'\n")
    required_if = [
        ("условие стоит у самой работы", {"pr-check.yml": WITH_JOB_IF}, True),
        ("условие стоит у шага — записи не создаёт", {"pr-check.yml": WITH_STEP_IF}, False),
        ("условия нет вовсе", {"pr-check.yml": "jobs:\n  check:\n    steps:\n      - run: echo\n"}, False),
        ("условие в НЕобязательном прогоне — не предмет",
         {"review.yml": WITH_JOB_IF}, False),
    ]
    for name, flws, must_reject in required_if:
        found = required_job_conditions(flws)
        if bool(found) is not must_reject:
            broken.append(f"{name}: ожидалось {'отказ' if must_reject else 'пропуск'}, вышло {found}")
        print(f"  {'отвергнут' if found else 'пропущен '} — обязательная запись: {name}")
    for name, srcs, must_reject in claims:
        found = claimed_rules(srcs, ANSWERS)
        if bool(found) is not must_reject:
            broken.append(f"{name}: ожидалось {'отказ' if must_reject else 'пропуск'}, вышло {found}")
        print(f"  {'отвергнут' if found else 'пропущен '} — {name}")
    ...
    pipeline_cases = [
        ...
        # Копии нет — значит нет и фильтра, а сверять нечего. Случай был
        # отказом до 7 сентября: тогда фильтр существовал, и его исчезновение
        # означало бы врущий комментарий. Теперь фильтр убран за дело —
        # пропуск по `if` давал на голове запись `skipped`, которой защита
        # ветки удовлетворяется наравне с зелёной, и #140 слилось красным.
        ("копии нет — значит нет и фильтра", LABELS_SRC, FLOW_NONE, False),
