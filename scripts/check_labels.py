...
def selftest() -> int:
    ...
    # зона» без имени зоны — это отказ, по которому нечего чинить (правило 158).
    ...
    # Вывод метки по заголовку — вход прогона `agent-pr.yml`, и ошибка здесь
    ...
    # Зоны, выводимые для `agent-pr.yml`, — это тот же derive_zones, которым
...
def main() -> int:
    ...
    if "--zones-for-diff" in sys.argv[1:]:
        # Вход прогона `agent-pr.yml`: пути изменённых файлов приходят потоком,
    ...
    if "--label-for-subject" in sys.argv[1:]:
        # Вход прогона `agent-pr.yml`. Печатается только метка: пусто значит
